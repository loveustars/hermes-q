...
def agg_to_clock(idx: pd.DatetimeIndex, r: np.ndarray, freq: str = "8h"):
    """按**绝对时间桶**把收益聚合到 8 小时。

    早期版本按"各自起点的第 k 根"聚合，但 carry 与基准的起点不同，
    8 小时网格整体错开，对齐后剩不到 10 个点 —— 回归退化成 t=0。
    按时钟分桶就没有这个问题：两边落在同一个绝对时间桶里。
    """
    s = pd.Series(np.asarray(r, dtype=float), index=pd.DatetimeIndex(idx))
    g = s.groupby(s.index.floor(freq))
    out = g.apply(lambda x: float(np.expm1(np.log1p(x).sum())))   # 复利聚合
...
def main() -> None:
    ...
    bm_r_8h, bm_idx_8h = agg_to_clock(bm_idx_1h, bm_r_1h, "8h")
    ...
    for s in syms:
        ...
        for lag in LAGS:
            at = protocol.alpha_vs_benchmark(r, bm_r_1h, BARS_PER_YEAR,
                                             ...
                                             b_index=bm_idx_1h)
            print(f"      滞后 {lag:>4} → t = {at.t_stat:>7.2f}   p = {at.p_value:.6f}"
                  f"   n = {at.n:,}")

        print(f"  (B) 按绝对时间桶聚合到 {AGG}h 再检验：")
        r8, idx8 = agg_to_clock(idx, r, f"{AGG}h")
        ...
        print(f"      alpha 年化 {at8.alpha_ann*100:>6.2f}%   beta {at8.beta:>6.3f}"
              f"   t = {at8.t_stat:>6.2f}   p = {at8.p_value:.6f}")
        print(f"      DSR = {ev8['dsr']['dsr']:.4f}   "
              f"bootstrap CI [{ev8['alpha_bootstrap']['annualized_ci_low']*100:.2f}%, "
        ...
        at_default = protocol.alpha_vs_benchmark(r, bm_r_1h, BARS_PER_YEAR,
                                                r_index=idx, b_index=bm_idx_1h)
        print(f"  对比：默认滞后 t={at_default.t_stat:.2f} → 聚合到 {AGG}h t={at8.t_stat:.2f}")
        if at8.t_stat > 0:
            print(f"  ⇒ 默认口径把 t 高估了约 "
                  f"{at_default.t_stat/at8.t_stat:.1f} 倍；以聚合口径为准，"
